...
export function AIChat() {
    ...
    const [messages, setMessages] = useState<Message[]>([
        {
            ...
            role: 'assistant',
            content:
                'Halo! Saya adalah AI Assistant Qalcuity. Saya bisa membantu Anda dengan:\n\n• Menampilkan data penjualan\n• Membuat invoice\n• Menganalisis profit & loss\n• Memberikan insight bisnis\n\nAda yang bisa saya bantu?',
            timestamp: new Date(),
        },
    ]);
    ...
    const handleSend = async () => {
        ...
        try {
            ...
            if (!res.ok) {
                ...
                throw new Error(errData.error || 'Gagal menghubungi AI service');
            }
    ...
    return (
        <div
            className={`fixed bottom-6 right-6 z-50 flex flex-col rounded-xl border bg-white shadow-2xl transition-all dark:border-gray-700 dark:bg-gray-800 ${isMinimized ? 'h-16 w-16' : 'h-[500px] w-96'
                }`}
        >
            {/* Header */}
            <div className="flex items-center justify-between border-b px-3 py-3 dark:border-gray-700">
                    ...
                    <div className="border-t p-3 dark:border-gray-700">
                        <div className="flex gap-2">
                            <input
                                ...
                                className="flex-1 rounded-lg border px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-100"
                            />
                            <button
                                onClick={handleSend}
                                disabled={!input.trim()}
