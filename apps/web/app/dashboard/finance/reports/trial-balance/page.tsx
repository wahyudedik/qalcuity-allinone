...
function exportToCsv(data: TrialBalanceData, t: (key: string) => string): void {
    const headers = [
        t('finance.reports.trialBalance.code'),
        t('finance.reports.trialBalance.accountName'),
        t('finance.reports.trialBalance.type'),
        t('finance.reports.trialBalance.debit'),
        t('finance.reports.trialBalance.credit'),
        t('finance.reports.trialBalance.balance'),
    ]
    ...
    const rows = data.accounts.map((acc) => [
        ...
        acc.totalDebit,
        acc.totalCredit,
        `${acc.balance} ${acc.balanceType === 'debit' ? 'D' : 'K'}`,
    ])
    ...
    const csvContent = [
        ...
        '',
        `${t('finance.reports.trialBalance.total')},,,${data.totalDebit},${data.totalCredit},${Math.abs(data.totalDebit - data.totalCredit)}`,
    ].join('\n')
...
export default function TrialBalancePage() {
    ...
    useEffect(() => {
        async function fetchData() {
            try {
                ...
                if (!res.ok) throw new Error(t('finance.reports.trialBalance.failedToLoad'))
                const json = await res.json()
                if (json.success && json.data) {
                    setData(json.data)
                } else {
                    throw new Error(json.error || t('finance.reports.trialBalance.errorOccurred'))
                }
            } catch (err) {
                setError(err instanceof Error ? err.message : t('finance.reports.trialBalance.errorOccurred'))
            } finally {
                setLoading(false)
            }
        }
    ...
    const accountTypeLabels: Record<string, string> = {
        ASSET: t('finance.reports.trialBalance.asset'),
        LIABILITY: t('finance.reports.trialBalance.liability'),
        EQUITY: t('finance.reports.trialBalance.equity'),
        REVENUE: t('finance.reports.trialBalance.revenue'),
        EXPENSE: t('finance.reports.trialBalance.expense'),
    }
        ...
        return (
            <div className="space-y-6">
                <div className="flex items-center gap-2">
                    ...
                    <h1 className="text-2xl font-bold text-gray-900 dark:text-white">{t('finance.reports.trialBalance.title')}</h1>
                </div>
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <button onClick={() => window.location.reload()} className="mt-2 text-sm text-red-600 hover:text-red-800 underline dark:text-red-400">
                                {t('finance.reports.trialBalance.retry')}
                            </button>
                        </div>
                    </div>
                </div>
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-3">
                    <label className="flex items-center gap-2 text-sm text-gray-600 dark:text-gray-400">
                        ...
                        />
                        {t('finance.reports.trialBalance.showZeroBalance')}
                    </label>
                    <button
                    ...
                    {data.isBalanced ? (
                        <span className="inline-flex items-center gap-1 rounded-full bg-green-100 px-3 py-1 text-xs font-medium text-green-800 dark:bg-green-900/30 dark:text-green-400">
                            ...
                            {t('finance.reports.trialBalance.balanced')}
                        </span>
                    ) : (
                        <span className="inline-flex items-center gap-1 rounded-full bg-red-100 px-3 py-1 text-xs font-medium text-red-800 dark:bg-red-900/30 dark:text-red-400">
                            ...
                            {t('finance.reports.trialBalance.unbalanced')}
                        </span>
                    )}
                </div>
            </div>
                    ...
                    <table className="w-full text-sm">
                        <thead>
                            <tr className="bg-gray-50 dark:bg-gray-800/50 border-b border-gray-200 dark:border-gray-700">
                                <th className="px-4 py-3 text-left font-medium text-gray-500 dark:text-gray-400">{t('finance.reports.trialBalance.code')}</th>
                                <th className="px-4 py-3 text-left font-medium text-gray-500 dark:text-gray-400">{t('finance.reports.trialBalance.accountName')}</th>
                                <th className="px-4 py-3 text-center font-medium text-gray-500 dark:text-gray-400">{t('finance.reports.trialBalance.type')}</th>
                                <th className="px-4 py-3 text-right font-medium text-gray-500 dark:text-gray-400">{t('finance.reports.trialBalance.debit')}</th>
                                <th className="px-4 py-3 text-right font-medium text-gray-500 dark:text-gray-400">{t('finance.reports.trialBalance.credit')}</th>
                                <th className="px-4 py-3 text-right font-medium text-gray-500 dark:text-gray-400">{t('finance.reports.trialBalance.balance')}</th>
                            </tr>
                        </thead>
                        <tbody>
                        ...
                        <tfoot>
                            <tr className="bg-gray-50 dark:bg-gray-800/50 border-t-2 border-gray-300 dark:border-gray-600 font-bold">
                                <td colSpan={3} className="px-4 py-3 text-right text-gray-900 dark:text-white">{t('finance.reports.trialBalance.total')}</td>
                ...
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <p className={`text-sm font-medium ${data.isBalanced ? 'text-green-800 dark:text-green-300' : 'text-red-800 dark:text-red-300'}`}>
                            {data.isBalanced
                                ? t('finance.reports.trialBalance.balancedMessage')
                                    .replace('{debit}', formatCurrency(data.totalDebit))
                                    .replace('{credit}', formatCurrency(data.totalCredit))
                                : t('finance.reports.trialBalance.unbalancedMessage')
                                    .replace('{diff}', formatCurrency(Math.abs(data.totalDebit - data.totalCredit)))}
                        </p>
                    </div>
                </div>
            </div>
            ...
            <p className="text-xs text-gray-400 dark:text-gray-500 text-right">
                {t('finance.reports.trialBalance.generatedAt')} {new Date(data.generatedAt).toLocaleString('id-ID')}
            </p>
        </div>
    )
}
