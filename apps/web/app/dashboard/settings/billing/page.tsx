        ...
        <div className="space-y-6">
            ...
            <div className="bg-white rounded-xl border border-gray-200 p-6">
                <div className="flex items-center justify-between mb-4">
                    <div>
                        <h2 className="text-lg font-semibold text-gray-900">{t('settings.currentPlanTitle') || 'Paket Saat Ini'}</h2>
                        <p className="text-sm text-gray-600 mt-1">{t('settings.manageSubscription') || 'Kelola langganan Anda'}</p>
                    </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-6 pt-4 border-t border-gray-200">
                    <div>
                        <div className="text-sm text-gray-600">{t('settings.monthlyBilling') || 'Tagihan Bulanan'}</div>
                        <div className="text-2xl font-bold text-gray-900 mt-1">{formatCurrency(799000)}</div>
                    </div>
                    <div>
                        <div className="text-sm text-gray-600">{t('settings.usersUsed') || 'User Digunakan'}</div>
                        <div className="text-2xl font-bold text-gray-900 mt-1">7 / 10</div>
                    </div>
                    <div>
                        <div className="text-sm text-gray-600">{t('settings.storage') || 'Storage'}</div>
                        <div className="text-2xl font-bold text-gray-900 mt-1">256MB / 1GB</div>
                    </div>
                    <div>
                        <div className="text-sm text-gray-600">{t('settings.nextPayment') || 'Pembayaran Berikutnya'}</div>
                ...
                <div className="flex gap-3 mt-6">
                    ...
                    <button className="px-4 py-2.5 border border-gray-300 rounded-lg text-sm font-medium text-gray-700 hover:bg-gray-50 transition-colors">
                        {t('settings.viewInvoice') || 'Lihat Invoice'}
                    </button>
                </div>
            </div>
            ...
            {/* Available Plans */}
            <div>
                <h2 className="text-lg font-semibold text-gray-900 mb-4">{t('settings.availablePlans') || 'Paket Tersedia'}</h2>
                        ...
                        >
                            ...
                            {currentPlan === plan.id && (
                                <div className="inline-block px-3 py-1 bg-green-100 text-green-700 text-xs font-medium rounded-full mb-4">
                                    {t('settings.yourPlan') || 'Paket Anda'}
                                </div>
                            )}
                            ...
                            >
                                {currentPlan === plan.id ? (t('settings.activePlan') || 'Paket Aktif') : (t('settings.selectPlan') || 'Pilih Paket')}
                            </button>
                        </div>
                    ))}
                </div>
            ...
            {/* Payment Method */}
            <div className="bg-white rounded-xl border border-gray-200 p-6">
                <h2 className="text-lg font-semibold text-gray-900 mb-4">{t('settings.paymentMethod') || 'Metode Pembayaran'}</h2>
                ...
                <button className="mt-4 text-sm text-blue-600 hover:text-blue-700 font-medium">
                    + {t('settings.addPaymentMethod') || 'Tambah Metode Pembayaran'}
                </button>
            </div>

            {/* Invoice History */}
            <div className="bg-white rounded-xl border border-gray-200 p-6">
                <h2 className="text-lg font-semibold text-gray-900 mb-4">{t('settings.invoiceHistory') || 'Riwayat Invoice'}</h2>
                <div className="overflow-x-auto">
                    <table className="w-full">
                        <thead>
                            <tr className="border-b border-gray-200">
                                <th className="text-left py-3 px-4 text-sm font-medium text-gray-600">Invoice</th>
                                <th className="text-left py-3 px-4 text-sm font-medium text-gray-600">{t('common.date') || 'Tanggal'}</th>
                                <th className="text-left py-3 px-4 text-sm font-medium text-gray-600">{t('common.amount') || 'Jumlah'}</th>
                                <th className="text-left py-3 px-4 text-sm font-medium text-gray-600">{t('common.status') || 'Status'}</th>
                                <th className="text-right py-3 px-4 text-sm font-medium text-gray-600">{t('common.actions') || 'Aksi'}</th>
                            </tr>
                        </thead>
                            ...
                            <tr>
                                ...
                                <td className="py-3 px-4">
                                    <span className="inline-flex px-2 py-1 text-xs font-medium bg-green-100 text-green-700 rounded-full">
                                        {t('settings.paidStatus') || 'Lunas'}
                                    </span>
                                </td>
                                <td className="py-3 px-4 text-right">
                                    <button className="text-sm text-blue-600 hover:text-blue-700">{t('settings.download') || 'Download'}</button>
                                </td>
                            </tr>
                            <tr>
