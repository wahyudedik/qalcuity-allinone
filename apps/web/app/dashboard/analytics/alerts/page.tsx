...
export default function AnalyticsAlertsPage() {
    ...
    const CONDITION_OPTIONS = [
        { value: 'below', label: t('analytics.alerts.conditions.below') },
        { value: 'above', label: t('analytics.alerts.conditions.above') },
        { value: 'equals', label: t('analytics.alerts.conditions.equals') },
        { value: 'not_equals', label: t('analytics.alerts.conditions.not_equals') },
        { value: 'changes_by', label: t('analytics.alerts.conditions.changes_by') },
    ]

    const METRIC_OPTIONS = [
        { id: 'revenue', name: t('analytics.alerts.metrics.revenue') },
        { id: 'total_expenses', name: t('analytics.alerts.metrics.totalExpenses') },
        { id: 'win_rate', name: t('analytics.alerts.metrics.winRate') },
        { id: 'pipeline_value', name: t('analytics.alerts.metrics.pipelineValue') },
        { id: 'total_stock_value', name: t('analytics.alerts.metrics.stockValue') },
        { id: 'employee_count', name: t('analytics.alerts.metrics.employeeCount') },
        { id: 'attendance_rate', name: t('analytics.alerts.metrics.attendanceRate') },
    ]
    ...
    const deleteRule = async (id: string) => {
        if (!confirm(t('analytics.alerts.deleteConfirm'))) return
        try {
        ...
        <div className="space-y-6">
            ...
            <div className="flex items-center gap-4 rounded-lg border border-gray-200 bg-white px-4 py-3 dark:border-gray-700 dark:bg-gray-800">
                <span className="text-sm text-gray-500 dark:text-gray-400">
                    {t('analytics.alerts.tabRules')} (<span className="font-medium text-gray-700 dark:text-gray-300">{activeRuleCount} {t('analytics.alerts.activeLabel')}</span>)
                </span>
                <span className="text-gray-300 dark:text-gray-600">|</span>
                <span className="text-sm text-gray-500 dark:text-gray-400">
                    {t('analytics.alerts.tabTriggers')} (<span className="font-medium text-gray-700 dark:text-gray-300">{totalTriggerCount} {t('analytics.alerts.totalLabel')}</span>)
                </span>
            </div>
            ...
            <div className="flex gap-2">
                ...
                >
                    ...
                    {t('analytics.alerts.tabTriggers')}
                </button>
            </div>
                                        ...
                                        <div className="flex items-start justify-between">
                                            <div className="min-w-0 flex-1">
                                                <div className="flex items-center gap-2">
                                                    ...
                                                    <span className={`inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-xs font-medium ${sevConfig.bg} ${sevConfig.color}`}>
                                                        ...
                                                        {t(`analytics.alerts.severities.${rule.severity}`)}
                                                    </span>
                                                </div>
                                                {rule.description && (
                                                ...
                                                <p className="mt-2 text-sm text-gray-600 dark:text-gray-300">
                                                    <span className="font-medium">{t('analytics.alerts.ifLabel')}</span> {rule.metricId} {getConditionLabel(rule.condition)} {rule.threshold}
                                                </p>
                                                <p className="mt-1 text-sm text-gray-600 dark:text-gray-300">
                                                    <span className="font-medium">{t('analytics.alerts.thenLabel')}</span> notify via {rule.notificationChannels.join(', ')}
                                                </p>
                                                <div className="mt-2 flex items-center gap-4 text-xs text-gray-400 dark:text-gray-500">
                                                    {rule.lastTriggeredAt && (
                                                        <span className="inline-flex items-center gap-1">
                                                            <Clock className="h-3 w-3" />
                                                            {t('analytics.alerts.lastTriggered')} {timeAgo(rule.lastTriggeredAt)}
                                                        </span>
                                                    )}
                                                    <span>{t('analytics.alerts.cooldownLabel')} {rule.cooldownMinutes} min</span>
                                                    <span>{rule.triggerCount} {t('analytics.alerts.triggersCount')}</span>
                                                </div>
                                            </div>
                                            ...
                                            <div className="ml-4 flex items-center gap-2">
                                                <button
                                                    ...
                                                    title={rule.isActive ? t('analytics.alerts.disable') : t('analytics.alerts.enable')}
                ...
                <>
                    {triggers.length === 0 ? (
                        <div className="flex flex-col items-center justify-center rounded-lg border border-dashed border-gray-300 py-12 dark:border-gray-600">
                            ...
                            <h3 className="mt-4 text-lg font-semibold text-gray-600 dark:text-gray-400">
                                {t('analytics.alerts.triggerEmpty.title')}
                            </h3>
                            <p className="mt-1 text-sm text-gray-400 dark:text-gray-500">
                                {t('analytics.alerts.triggerEmpty.description')}
                            </p>
                        </div>
                    ) : (
                                    ...
                                    >
                                        ...
                                        <div className="min-w-0 flex-1">
                                            ...
                                            <div className="mt-0.5 flex items-center gap-3 text-xs text-gray-400 dark:text-gray-500">
                                                ...
                                                {trigger.acknowledged && (
                                                    <span className="text-green-600 dark:text-green-400">{t('analytics.alerts.acknowledged')}</span>
                                                )}
                                            </div>
                                        </div>
                                        {!trigger.acknowledged && (
                                            ...
                                            >
                                                {t('analytics.alerts.acknowledge')}
                                            </button>
                                        )}
                                    </div>
                                )
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
                    <div className="w-full max-w-md rounded-lg bg-white p-6 shadow-xl dark:bg-gray-800">
                        ...
                        <div className="mt-4 space-y-4">
                            <div>
                                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{t('analytics.alerts.form.name')} *</label>
                                <input
                                    type="text"
                                    value={form.name}
                            ...
                            <div>
                                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{t('analytics.alerts.form.description')}</label>
                                <input
                                    type="text"
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{t('analytics.alerts.form.metric')} *</label>
                                    <select
                                        value={form.metricId}
                                ...
                                <div>
                                    <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{t('analytics.alerts.form.condition')} *</label>
                                    <select
                            ...
                            <div className="grid grid-cols-3 gap-4">
                                <div>
                                    <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{t('analytics.alerts.form.threshold')} *</label>
                                    <input
                                        type="number"
                                ...
                                <div>
                                    <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{t('analytics.alerts.form.severity')}</label>
                                    <select
                                        value={form.severity}
                                    ...
                                    >
                                        {SEVERITY_OPTIONS.map(s => (
                                            <option key={s} value={s}>{t(`analytics.alerts.severities.${s}`)}</option>
                                        ))}
                                    </select>
                                </div>
                                <div>
                                    <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{t('analytics.alerts.form.cooldown')}</label>
                                    <input
                                        type="number"
